...
def get_db():
    db = SessionLocal()
    try:
        yield db #return gibi
    finally:
        db.close() #session kapatır.
...
async def create_todo(user: user_dependency, db: db_dependency, todo_request: TodoRequest):
    if user is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
    todo = Todo(**todo_request.dict(), owner_id=user.get('id')) #kullanıdan alınan id user_id 'ye verilecek.
...
def create_todo_with_gemini(todo_string: str):
    ...
    response = llm.invoke(
        [
            HumanMessage(content="I will provide you a todo item to add my to do list. What i want yo to do is to create a longer and more compherensive description of that todo item, my next message will be my todo:"),
